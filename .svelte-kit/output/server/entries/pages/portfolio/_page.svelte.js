...
const ProjectCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.index === void 0 && $$bindings.index && index !== void 0)
    $$bindings.index(index);
  $$unsubscribe_theme();
  return `<div class="${"group relative overflow-hidden rounded-xl border " + escape($theme.border.default, true) + " " + escape($theme.bg.card, true) + " shadow-lg transition hover:-translate-y-1 hover:shadow-2xl flex flex-col h-full"}"> <div class="${"relative h-48 overflow-hidden " + escape($theme.bg.cardHover, true)}"><img${add_attribute("src", project.image, 0)}${add_attribute("alt", project.title, 0)} class="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy"> <div class="absolute top-2 left-2 flex gap-2 opacity-50 transition group-hover:opacity-100">${project.demo ? `<a${add_attribute("href", project.demo, 0)} target="_blank" rel="noopener noreferrer" class="${"inline-flex rounded-lg border " + escape($theme.border.light, true) + " " + escape($theme.bg.cardHover, true) + " p-2 " + escape($theme.text.muted, true) + " hover:" + escape($theme.border.hover, true)}"><img src="https://api.iconify.design/mdi:open-in-new.svg?color=%23cbd5e1" alt="Demo" class="h-4 w-4"></a>` : ``} ${project.github ? `<a${add_attribute("href", project.github, 0)} target="_blank" rel="noopener noreferrer" class="${"inline-flex rounded-lg border " + escape($theme.border.light, true) + " " + escape($theme.bg.cardHover, true) + " p-2 " + escape($theme.text.dim, true) + " hover:" + escape($theme.border.hover, true)}"><img src="https://imgs.search.brave.com/w5LFW4ei3PC6DUOkw2jcpG1OVDzoYhDqENlECFBWUg8/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly91cGxv/YWQud2lraW1lZGlh/Lm9yZy93aWtpcGVk/aWEvY29tbW9ucy8y/LzI0L0dpdGh1Yl9s/b2dvX3N2Zy5zdmc" alt="GitHub" class="h-4 w-4"></a>` : ``}</div></div> <div class="p-5 flex flex-col "> <div class="flex items-start justify-between gap-3 min-h-[5rem] mb-2"><div class=""><h3 class="${"text-2xl font-semibold " + escape($theme.text.secondary, true) + " transition"}">${escape(project.title)}</h3> <p class="${"text-sm uppercase tracking-[0.35em] " + escape($theme.text.muted, true) + " mt-1"}">${escape(project.category.join(", "))}</p></div> <div class="flex flex-col items-end gap-2 flex-shrink-0"><span class="${"rounded px-2 py-1 text-xs font-semibold uppercase tracking-[0.25em] " + escape($theme.bg.secondary, true) + " " + escape($theme.text.primary, true)}">${escape(project.duration)}</span> <span${add_attribute(
    "class",
