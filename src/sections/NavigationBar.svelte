...
<nav class="w-full bg-black/80 backdrop-blur-sm border-b border-emerald-500/40 px-4 sm:px-8 top-0 z-50 sticky">
	<div class="mx-auto flex h-16 w-full max-w-7xl items-center gap-6">
		<a href="/" class="flex flex-shrink-0 items-center gap-3" aria-label="Go to home">
			<Logo class="w-10 h-10 transition-transform duration-300 hover:-translate-y-1" />
			<span class="hidden sm:block text-sm font-semibold tracking-[0.4em] text-emerald-400 uppercase">
				Akhil Gogineni
			</span>
		</a>
		...
		<div class="ml-auto flex items-center gap-6">
			...
			<div class="flex items-center gap-3">
				<div class="lg:hidden">
					...
					>
						{#each navItems as item}
							<option value={item.href} selected={$page.url.pathname === item.href}>
								{item.label}
							</option>
						{/each}
					</select>
